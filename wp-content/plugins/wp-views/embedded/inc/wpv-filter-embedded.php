...
function wpv_filter_shortcode_submit($atts){
	...
	global $WP_Views;
	$view_name = get_post_field( 'post_name', end($WP_Views->view_used_ids));
 ...
 * Link:
 *
 * Note: This shortcode is deprecated in favour of the new wpv-items-found.
 *
 */
 ...
 * Link:
 *
 * Note: This shortcode is deprecated in favour of the new wpv-no-items-found.
 *
 */
 ...
 *
 * Example usage:
 * [wpv-items-found]Some posts/taxonomy terms were found[/wpv-items-found]
 *
 * Link:
 *
 * Note:
...
class Walker_Category_radios extends Walker {
	...
	function start_el(&$output, $category, $depth = 0, $args = array(), $current_object_id = 0) {
        ...
        if ($this->slug_mode) {
    		...
    		$output .= '<span class="radios-taxonomies-title">' . $indent . $tax_option . '</span><input name="'.$name.'" type="radio" value="' . $category->slug. '"' . $selected . '/>' . "\n";
    ...
    if(!function_exists('wpv_terms_checklist')){
        function wpv_terms_checklist($post_id = 0, $args = array()) {
            ...
            if ( empty($walker) || !is_a($walker, 'Walker') )
                $walker = new WPV_Walker_Category_Checklist($url_format);
