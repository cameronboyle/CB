...
if(is_admin()){
	...
	function wpv_add_filter_category_list_item($view_settings) {
		...
		if ($td != '') {
			echo "<li id='js-row-taxonomy' class='filter-row-multiple js-filter-row js-filter-row-multiple js-filter-for-posts js-filter-taxonomy js-filter-row-taxonomy'>";
				echo "<p class='edit-filter js-wpv-filter-edit-controls'>\n<button class='button-secondary edit-trigger js-wpv-filter-edit-open'>".__('Edit','wpv-views')."</button>\n<i class='icon-remove-sign js-filter-taxonomy-row-remove' data-nonce='". wp_create_nonce( 'wpv_view_filter_taxonomy_row_delete_nonce' ) . "'></i>\n</p>";
				if ($summary != '') {
					echo "<p class='wpv-filter-taxonomy-edit-summary js-wpv-filter-summary js-wpv-filter-taxonomy-summary'>";
					_e('Select posts with taxonomy: ', 'wpv-views');
					echo $summary;
					echo "</p>\n";
				}
				echo $toolset_alert;
				echo "<div id=\"wpv-filter-taxonomy-edit\" class=\"wpv-filter-edit js-filter-taxonomy-edit js-wpv-filter-edit\">\n";
				echo $td;?>
					<div class="wpv-filter-taxonomy-relationship js-wpv-filter-taxonomy-relationship">
						<p>
							<strong><?php _e('Taxonomy relationship:', 'wpv-views') ?></strong>
						</p>
						<p>
		...
		?>
			<div class="wpv-taxonomy-edit-row wpv-filter-row-multiple-element js-filter-row-multiple-element js-filter-row-taxonomy-<?php echo $taxonomy; ?> js-filter-row-tax-<?php echo $type; ?>" data-taxonomy="<?php echo $taxonomy; ?>">
				...
				<p class="edit-filter js-wpv-filter-taxonomy-controls">
					<i class="icon-remove-sign js-filter-remove" data-nonce="<?php echo wp_create_nonce( 'wpv_view_filter_taxonomy_delete_nonce' );?>"></i>
				</p>

				<p>
					<strong><?php echo __('Taxonomy', 'wpv-views') . ' - ' . $taxonomy_name; ?>:</strong>
				</p>
				...
				<ul id="taxonomy-<?php echo $taxonomy; ?>" class="categorychecklist form-no-clear js-taxonomy-checklist">
					<?php wp_terms_checklist(0, array('taxonomy' => $taxonomy, 'selected_cats' => $cats_selected)) ?>
				</ul>

				<ul id="taxonomy-<?php echo $taxonomy; ?>-attribute-url" class="js-taxonomy-parameter">
					<li>
	...
	function wpv_filter_taxonomy_update_callback() {
		...
		echo wpv_get_filter_author_summary_text($filter_author);*/
