    ...
    <div class="wrap">
		...
		<div class="wpv-setting-container">
			...
			<div class="wpv-settings-header">
				<h3><?php _e('Export Views and Content Templates', 'wpv-views'); ?></h3>
			</div>

			<div class="wpv-setting">
		        <form name="View_export" action="<?php echo admin_url('edit.php'); ?>" method="post">

		            <h4><?php _e('Download all Views and Content Templates', 'wpv-views'); ?></h4>
    ...
    switch($type)
    {
		...
		$attached_images_sizes=array_merge(
			...
			if ('module_manager' == $mode ) {
				...
				$items_hash[$post['ID']] = md5(serialize(ksort_by_string_views($hash_data)));
			}
                    }
                }
            }
...
function ksort_by_string_views( $data ) {
	if ( is_array( $data ) ) {
		ksort( $data, SORT_STRING );
		foreach ( $data as $k => $v ) {
			$data[$k] = ksort_by_string_views( $v );
		}
	}
	return $data;
}
